...
let main =
  foreign
    ...
    "Unikernel.Main" (random @-> time @-> mclock @-> pclock @-> stackv4v6 @-> kv_rw @-> job)
...
let () = register "mirage_sshfs" [ main $ default_random $ default_time $ default_monotonic_clock $ default_posix_clock $ stack $ my_fs]
